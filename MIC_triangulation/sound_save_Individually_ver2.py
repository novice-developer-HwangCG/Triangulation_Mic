import numpy as np
import pyaudio
import wave
import time
from datetime import datetime

# 마이크 설정
MIC_A_INDEX = 3  # 마이크 A
MIC_B_INDEX = 2  # 마이크 B
MIC_C_INDEX = 1  # 마이크 C

# 마이크 위치 정보
MIC_LOCATIONS = {
    "middle": MIC_A_INDEX,
    "left": MIC_B_INDEX,
    "right": MIC_C_INDEX
}

# 상수 설정
CHUNK = 1024  # 오디오 버퍼 크기
RATE = 48000  # 샘플링 레이트
RECORD_SECONDS = 3600  # 최대 녹음 시간 (1시간)

# 현재 날짜 및 시간을 기반으로 파일 이름 설정
current_time = datetime.now().strftime("%d_%m_%y_%H:%M:%S")
LOG_FILENAME = f"sound_detection_{current_time}.log"
WAVE_FILENAMES = {
    location: f"recorded_{location}_{current_time}.wav"
    for location in MIC_LOCATIONS.keys()
}

# PyAudio 초기화
p = pyaudio.PyAudio()

# 마이크 스트림 열기 (3개 마이크)
streams = {
    location: p.open(format=pyaudio.paInt16, channels=1, rate=RATE, input=True,
                     input_device_index=index, frames_per_buffer=CHUNK)
    for location, index in MIC_LOCATIONS.items()
}

print("Start recording... (Ctrl+C to stop)")

def log_message(message):
    """ 로그 메시지를 파일에 저장 """
    with open(LOG_FILENAME, "a") as log_file:
        log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - {message}\n")

try:
    frames = {location: [] for location in MIC_LOCATIONS.keys()}
    
    start_time = time.time()
    while time.time() - start_time < RECORD_SECONDS:
        for location, stream in streams.items():
            data = stream.read(CHUNK, exception_on_overflow=False)
            frames[location].append(data)
        
        time.sleep(CHUNK / RATE)  # 일정한 샘플링 간격 유지

except KeyboardInterrupt:
    print("Recording stopped.")
    log_message("Recording stopped.")

finally:
    print("Saving recorded files...")
    
    def save_wave(filename, frames):
        wf = wave.open(filename, 'wb')
        wf.setnchannels(1)
        wf.setsampwidth(p.get_sample_size(pyaudio.paInt16))
        wf.setframerate(RATE)
        wf.writeframes(b''.join(frames))
        wf.close()
        print(f"Saved: {filename}")
    
    for location, filename in WAVE_FILENAMES.items():
        save_wave(filename, frames[location])
    
    # 스트림 종료
    for stream in streams.values():
        stream.stop_stream()
        stream.close()
    p.terminate()
    
    print("All recordings saved successfully.")
